#!/usr/bin/env python3
"""
LINE alerts for SPACEZ TERMINAL.

Sibling of alerts_telegram.py - same snapshot, same message content, same
daily-digest scheduling, just delivered as a LINE Official Account broadcast
instead of a Telegram message. Reads the snapshot that scripts/fetch_market.py
already produces (data/market.json).
...
CREDENTIALS
-----------
The channel access token comes from the environment only, supplied by GitHub
Actions from repository Secrets. It is never read from a file, never logged,
and must never be written into this repository - it is public.

    LINE_CHANNEL_ACCESS_TOKEN   long-lived channel access token, issued from
                                the LINE Developers Console for the
                                Messaging API channel under your OA

With it missing the script prints what it would have sent and exits 0, so
the data pipeline is never broken by a messaging problem. Delivery is a
BROADCAST - it goes to everyone who has added the LINE Official Account as
a friend, same as the "publish via LINE" button on the Print Report page -
there is no concept of a single recipient chat id the way Telegram has one.
...
STATE
-----
data/alerts_state_line.json remembers what has already gone out, so a signal
that stays true for days does not re-notify every 30 minutes. Kept separate
from alerts_state.json (Telegram's) so the two channels never race each
other's "already sent today" markers. The workflow commits it back alongside
the snapshot.
"""
...
STATE_FILE = os.path.join(ROOT, "data", "alerts_state_line.json")
...
# The watchlist file is keyed by the owner's Telegram chat id (see
# load_watchlist() below) - a LINE broadcast has no per-recipient id of its
# own, so this script just reads that SAME owner bucket rather than needing
# a second copy of the watchlist. Nothing is sent TO Telegram from here.
OWNER_KEY = os.environ.get("TELEGRAM_CHAT_ID", "").strip()

DRY_RUN = os.environ.get("ALERT_LINE_DRY_RUN", "").strip() == "1" or not TOKEN
...
def load_watchlist():
    ...
    if isinstance(users, dict):
        bucket = users.get(str(OWNER_KEY)) or {}
...
LINE_TEXT_LIMIT = 4900  # LINE's hard cap is 5000 chars per text message


def strip_html_for_line(text):
    """The shared message-building functions above write Telegram-style HTML
    (<b>, <i>, <code>, <a href>...) - LINE's plain "text" message type has no
    such markup, so this turns it into readable plain text instead of
    sending literal angle brackets to someone's phone."""
    text = _HTML_LINK_RE.sub(lambda m: "%s (%s)" % (m.group(2), m.group(1)), text)
    text = _HTML_TAG_RE.sub("", text)
    text = (text.replace("&amp;", "&").replace("&lt;", "<")
                .replace("&gt;", ">").replace("&quot;", '"'))
    if len(text) > LINE_TEXT_LIMIT:
        text = text[: LINE_TEXT_LIMIT - 20].rstrip() + "\n... (\u0e15\u0e31\u0e14\u0e02\u0e49\u0e2d\u0e04\u0e27\u0e32\u0e21\u0e40\u0e01\u0e34\u0e19)"
    return text


def send(text):
    """Broadcast one message to every LINE friend of the OA. Never raises -
    a messaging failure must not fail the data pipeline, and must never
    print the token."""
    plain = strip_html_for_line(text)
    if DRY_RUN:
        print("\n----- would broadcast (LINE) -----")
        print(plain)
        print("----------------------------------")
        return True
    payload = json.dumps({"messages": [{"type": "text", "text": plain}]}).encode("utf-8")
    ...
    try:
        ...
        try:
            ...
            pass
        print("line HTTP %s %s" % (exc.code, detail))
        return False
    except Exception as exc:  # network, DNS, timeout
        print("line unreachable: %s" % type(exc).__name__)
        return False
